...
//png _struct est une structure interne qui sera la premeire variable a donne a chaque appel de fonctions de libpng
...
void lecture_image_info(FILE * fp,image_info donnee_image, png_structp png_ptr,png_infop info_ptr,png_infop end_info){
...
type_pixel **lecture_image_pixel(png_structp png_ptr,png_infop info_ptr,image_info donnee_image){
    png_infop end_info = NULL;
    png_bytep * ligne_pointeur;
    int x, y;

    FILE *fp=init_fichier("pngnow.png");
    lecture_image_info(fp, donnee_image,png_ptr,info_ptr,end_info);
    type_pixel **matrice;

    matrice = malloc(donnee_image.largeur * sizeof(*matrice));       //On alloue 'taille1' pointeurs.
    if(matrice == NULL){
    ...
    for(int i=0 ; i < donnee_image.largeur ; i++){
        matrice[i] = malloc(donnee_image.hauteur * sizeof(**matrice) );       //On alloue des tableaux de 'taille2' variables.
        if(matrice[i] == NULL){  
            fprintf(stderr,"error:[matrice] l'alllocation pour la matrice a failli \n");
            png_destroy_read_struct(&png_ptr, &info_ptr,(png_infopp)NULL);
            fclose(fp);
...
int main() {
    ...
    tab = lecture_image_pixel(png_ptr, info_ptr, data_image);
